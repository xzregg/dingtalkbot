...
class API:
    ...
    @staticmethod
    def __generate_wrap(queue: block_queue.Queue, thread, event):
        while True:
            try:
...
class ChatGPT(API):
    ...
    def talk(self, prompt, model, message_id, parent_message_id, conversation_id=None, stream=True, token=None, system_prompt=''):
